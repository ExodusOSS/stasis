...
test('CLI: bundle writes a brotli-compressed Bundle to stasis.bundle.br by default when no -o is given', withTmp((t, tmp) => {
  ...
  const buf = readFileSync(join(tmp, 'stasis.bundle.br'))
...
test('CLI: bundle writes a brotli-compressed Bundle to stdout with --output=-', (t) => {
  ...
  t.assert.ok(!existsSync(join(fixtures, 'basic', 'stasis.bundle.br')), '--output=- must not write a file')
...
test('CLI: bundle summary names stasis.bundle.br as the destination when no -o is given', withTmp((t, tmp) => {
  cpSync(join(fixtures, 'basic'), tmp, { recursive: true })
  const r = runCli(['bundle', 'src/A.sol'], { cwd: tmp })
  t.assert.equal(r.status, 0, `stderr: ${r.stderr}`)
  t.assert.match(r.stderr, /\[stasis\] Bundled 2 files in 1 package from src to stasis\.bundle\.br/)
}))
