...
| `stasis bundle --metro --platforms=ios,android app.js` | statically bundle the way Metro resolves: RN conditions + mainFields + `.ios`/`.android`/`.native` suffixes, all platforms at once; dynamic `import()` pulls in Metro's injected async-import helper (`metro-runtime/src/modules/asyncRequire`) |
| `stasis bundle --metro --platforms=ios --asyncRequireModulePath=expo/internal/async-require-module app.js` | same, mirroring a Metro config that overrides `transformer.asyncRequireModulePath` (e.g. Expo) |
