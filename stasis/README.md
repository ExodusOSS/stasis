...
| `stasis bundle --nextjs` | statically bundle a Next.js app: discovers the convention entries itself (`pages/**`, `app/` special files, `middleware`, `instrumentation`, `src/` variants) and resolves them the way Next's compilers do — a server pass (`main`/`module`) and a client pass (`browser` mainFields + condition + browser-field redirects) whose entry set is the pages routes plus every reached `'use client'` boundary, with divergent resolutions recorded per pass (`{ client: …, server: … }`). Implies `--jsx` and `--typescript` (tsconfig or jsconfig `paths` honored); combines with `--resources`/`--package-json`/`--lockfile`, and extra entries (a custom server) may be listed. See [Next.js](https://github.com/ExodusOSS/stasis/blob/main/doc/nextjs.md) — the `withStasis` wrapper there captures real `next build`s through the same artifacts |
