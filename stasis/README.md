...
| `stasis add src assets` | same, expanding each directory to every file under it minus an auto-excluded set (type declarations, `.env` secrets, stasis's own outputs, docs/config/sidecar/log noise, VCS/sample-app/test-scaffolding/Apple-slice dirs); naming a path explicitly overrides the exclusions |
