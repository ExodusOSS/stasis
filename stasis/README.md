...
| `stasis bundle --pnpm src/index.js` | statically bundle from `pnpm-lock.yaml` alone, with no `node_modules` on disk and no install: every package tarball is downloaded into a cache (`--pnpm-cache=dir`), integrity-verified, unpacked in memory and laid out as pnpm's virtual `node_modules`, then scanned by the same rules — byte-identical to bundling a real `pnpm install`; no package script runs. Needs the optional [`@exodus/stasis-deps`](../stasis-deps) dependency. Combines with `--lockfile`/`--scope`/`--conditions`/`--mainFields`/`--metro`/`--jsx`/`--flow`/`--typescript`/`--resources`/`--package-json` (not `--metro-resolver`). See [pnpm.md](https://github.com/ExodusOSS/stasis/blob/main/doc/pnpm.md) |
| `stasis bundle --pnpm --pnpm-offline src/index.js` | same, refusing to download: every tarball must already be in the cache (a seeded, network-free build) |
...
The zero-dependency [`@exodus/stasis-core`](../stasis-core) CLI provides `run`, `prune`, and `add` commands only; the bundler plugins live in [`@exodus/stasis-plugins`](../stasis-plugins), and the pnpm-lockfile-to-in-memory-`node_modules` implementation behind `stasis bundle --pnpm` in the optional [`@exodus/stasis-deps`](../stasis-deps).
