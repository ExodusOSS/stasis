...
| `stasis add src assets` | same, expanding each directory to every file under it, minus an auto-excluded set: type declarations, `.env` secrets, stasis's own outputs, the non-build-input noise the native capture skips (docs/legal, editor/lint/CI config, `*.map`/`*.js.flow` sidecars, logs), and dirs it won't descend into (`.git`, `.github`, `.settings`, `example`/`examples`, `__tests__`, `__mocks__`, `jest`, Apple prebuilt slice dirs). What survives still has to be recognized source or a declared `resources` extension; naming a path explicitly overrides the exclusions |
