...
import { assertRealPathWithinBase, classifyExtension, moduleFileKey, splitNodeModulesPath } from '@exodus/stasis-core/util'
import {
  buildSolidityTree,
...
// build-intermediates dir), plus nested deps and VCS metadata -- never source.
...
function walkNativeDir(dirAbs, out) {
  ...
  for (const ent of entries) {
    ...
    if (ent.isDirectory()) {
      if (!NATIVE_SKIP_DIRS.has(ent.name)) walkNativeDir(full, out)
    } else if (ent.isFile()) {
      out.push(full)
    }
  }
...
// The native build-input files a bundled React Native dependency contributes to the app's
// native build: its podspec(s) at the package root, and every file under ios/ and android/.
// These are what CocoaPods/Xcode and Gradle consume; no JS module graph reaches them.
// Returns absolute paths (a non-native package simply yields none).
