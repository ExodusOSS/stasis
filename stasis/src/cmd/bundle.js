...
async function buildResolvedJsBundle({ cwd = process.cwd(), entries, mainFields, platforms, conditions = [], metro = false }) {
  ...
  // Native build-input source (podspec/gradle/Java/Kotlin/ObjC++/template/xml) is stored as CODE
  // under a source-language tag; other native assets as a resource ('resource', or
  // 'resource:base64' for a non-UTF-8 asset like a font/image); the integrity hashes the raw
  // bytes, matching the JS path.
  if (metro) {
    ...
    for (const pkgDir of [...pkgDirs].toSorted()) {
      ...
      for (const abs of files) {
        ...
        // Native build-input SOURCE (java/kotlin/gradle/objcpp/podspec/template/xml) is code under
        // a source-language tag; every other native file (ObjC .m/.h, C++, Swift, images, ...) is
        // an asset resource ('resource', or 'resource:base64' for non-UTF-8 bytes).
