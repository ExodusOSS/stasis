...
export class State {
  ...
  addFsDir(url, names) {
    ...
    if (this.config.childProcess) this.#observed.add(file) // only a child's shardSnapshot reads it; skip when the channel is off
    if (this.config.bundle) {
      this.#recordReason('run', file) // directory captures come only from the loader's --fs
      if (this.#pluginAttached) this.#fsReadPostPlugin.add(file) // fs-read; droppable from 'run' if a plugin bundles it
    }
    const rel = relative(dir, file)
    assert.ok(!rel.startsWith('..'))
  ...
  // works across stasis-core copies (the plugin may be a foreign-copy instance), like addSidecar.
  markPluginAttached() {
