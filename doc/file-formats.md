...
- `formats` records each file's format in the same shape as the bundle's
  ...
  Node; plus `pbxproj` for an Xcode project file added via `stasis add`, since the packager
  excludes the `.xcodeproj` bundle it lives in). Resource files use `resource` (content is
...
- `formats`: project-relative path → format. Code files use a Node loader
  ...
  `template`, `xml`, `env`, `fastlane` — the Metro native capture's build-input surface; plus
  `pbxproj` for an Xcode project file added via `stasis add`).
