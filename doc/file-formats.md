...
- `imports` records observed resolutions (conditions → parent file → specifier →
  ...
  in `scope = node_modules`. Lockfiles predating this field omit it (frozen runs warn).
...
- `formats` records each file's format. Values:
  ...
  `full`). Lockfiles predating this field omit it (frozen runs warn).
...
- When the lockfile records `imports`, every bundle resolution edge must land on
  ...
  or inconsistently-attested edges are fatal. Lockfiles without `imports` skip the
  check (`lock = frozen` warns); regenerate to enable it.
