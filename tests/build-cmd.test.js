...
const runCli = (args, { cwd } = {}) => {
  const r = spawnSync(process.execPath, [cli, ...args], { encoding: 'utf-8', cwd, env: cleanEnv })
...
test('build from a full-scope bundle produces a runnable output', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  const bundled = runCli(['bundle', '--output=app.code.br', 'src/entry.js'], { cwd: tmp })
  t.assert.equal(bundled.status, 0, bundled.stderr)

  const r = runCli(['build', '--output=out.js', 'app.code.br'], { cwd: tmp })
...
test('build from a lockfile reads and verifies sources from disk', withTmp((t, tmp) => {
  // The fixture ships a committed lockfile (graph + per-file digests, no content).
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--output=out.js', 'stasis.lock.json'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
  t.assert.match(r.stderr, /Built src\/entry\.js from lockfile/)
  t.assert.equal(runNode(t, join(tmp, 'out.js')).trim(), 'hello, world')
}))

test('build from a bundle is self-contained (no sources on disk)', withTmp((t, tmp) => {
  // Phase 1: bundle in a source dir.
  const cap = join(tmp, 'cap')
  cpSync(fullFixture, cap, { recursive: true })
  t.assert.equal(runCli(['bundle', '--output=app.code.br', 'src/entry.js'], { cwd: cap }).status, 0)
  ...
  const r = runCli(['build', '--output=out.js', 'app.code.br'], { cwd: load })
  t.assert.equal(r.status, 0, r.stderr)
  t.assert.equal(runNode(t, join(load, 'out.js')).trim(), 'hello, world')
}))

test('build follows the bundle\'s recorded bytes, not whatever is on disk', withTmp((t, tmp) => {
  const cap = join(tmp, 'cap')
  cpSync(fullFixture, cap, { recursive: true })
  t.assert.equal(runCli(['bundle', '--output=app.code.br', 'src/entry.js'], { cwd: cap }).status, 0)
  ...
  const r = runCli(['build', '--output=out.js', 'app.code.br'], { cwd: build })
  t.assert.equal(r.status, 0, r.stderr)
...
test('build from a lockfile fails closed when a source was changed on disk', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
...
test('build rejects a node_modules-scope artifact', withTmp((t, tmp) => {
...
test('build rejects a non-JS (Solidity) bundle', withTmp((t, tmp) => {
  cpSync(solFixture, tmp, { recursive: true })
  t.assert.equal(runCli(['bundle', '--output=s.code.br', 'src/A.sol'], { cwd: tmp }).status, 0)

  const r = runCli(['build', '--output=out.js', 's.code.br'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
  t.assert.match(r.stderr, /JavaScript\/TypeScript/)
}))

test('build of an entry that imports an asset fails (assets are not buildable yet)', withTmp((t, tmp) => {
...
test('build a code-only entry from an artifact that also carries assets for another entry', withTmp((t, tmp) => {
  ...
  const r = runCli(['build', '--output=out.js', 'app.code.br', 'src/code.js'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
  t.assert.equal(runNode(t, join(tmp, 'out.js')).trim(), 'hello, world')
}))

test('build is artifact-driven: a conflicting project stasis.config.json is ignored', withTmp((t, tmp) => {
...
test('build a bundle in a bare dir with no package.json (self-contained, anchors at the artifact dir)', withTmp((t, tmp) => {
  ...
  const r = runCli(['build', '--output=out.js', 'app.code.br'], { cwd: bare })
  t.assert.equal(r.status, 0, r.stderr)
  t.assert.equal(runNode(t, join(bare, 'out.js')).trim(), 'hello, world')
}))

test('build ignores a stray stasis.code.br in the tree (no disk bundle discovery)', withTmp((t, tmp) => {
...
test('build forwards --format and --minify to esbuild', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })

  const r = runCli(['build', '--format=cjs', '--minify', '--output=out.cjs', 'stasis.lock.json'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
...
test('build forwards --sourcemap (emits a .map alongside the output)', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--sourcemap', '--output=out.js', 'stasis.lock.json'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
...
test('build writes into a directory --output (esbuild names the file from the entry)', withTmp((t, tmp) => {
  ...
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--output=dist', 'stasis.lock.json'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
...
test('build refuses to overwrite a source the artifact records (no data loss)', withTmp((t, tmp) => {
  ...
  const r = runCli(['build', '--output=src/hello.js', 'stasis.lock.json'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
...
test('build forwards --define to esbuild', withTmp((t, tmp) => {
  ...
  const r = runCli(['build', '--define=__DEV__=false', '--output=out.js', 'app.code.br'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
  // __DEV__ -> false, so the constant branch folds to 'PROD'.
  t.assert.equal(runNode(t, join(tmp, 'out.js')).trim(), 'PROD')
}))

test('build rejects a malformed --define', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--define=NOPE', '--output=out.js', 'stasis.lock.json'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
  t.assert.match(r.stderr, /--define must be KEY=VALUE/)
}))

test('build externalizes a non-recorded specifier only when --external names it', withTmp((t, tmp) => {
  ...
  const fail = runCli(['build', '--format=cjs', '--output=out.cjs', 'app.code.br'], { cwd: tmp })
  ...
  const ok = runCli(['build', '--format=cjs', '--external=ext-pkg', '--output=out.cjs', 'app.code.br'], { cwd: tmp })
...
test('build forwards --loader to enable JSX in a .js file', withTmp((t, tmp) => {
  ...
  const ok = runCli(['build', '--format=cjs', '--loader=.js:jsx', '--output=out.cjs', 'app.code.br'], { cwd: tmp })
  t.assert.equal(ok.status, 0, ok.stderr)
  t.assert.equal(runNode(t, join(tmp, 'out.cjs')).trim(), '<p>hi</p>')
}))

test('build rejects a malformed --loader', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--loader=jsx', '--output=out.js', 'stasis.lock.json'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
...
test('build defaults to the sole entry when the artifact has one', withTmp((t, tmp) => {
  // Covered implicitly elsewhere, but assert the no-entry-arg path explicitly.
  cpSync(fullFixture, tmp, { recursive: true })
  t.assert.equal(runCli(['bundle', '--output=app.code.br', 'src/entry.js'], { cwd: tmp }).status, 0)
...
test('build accepts an explicit entry for a single-entry artifact', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--output=out.js', 'stasis.lock.json', 'src/entry.js'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
  t.assert.equal(runNode(t, join(tmp, 'out.js')).trim(), 'hello, world')
}))

test('build requires an entry when the artifact has several, and lists them', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  // Bundle two entry points (hello.js is a valid module entry of its own).
  t.assert.equal(runCli(['bundle', '--output=app.code.br', 'src/entry.js', 'src/hello.js'], { cwd: tmp }).status, 0)
...
test('build builds the selected entry from a multi-entry artifact', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  t.assert.equal(runCli(['bundle', '--output=app.code.br', 'src/entry.js', 'src/hello.js'], { cwd: tmp }).status, 0)

  const r = runCli(['build', '--output=out.js', 'app.code.br', 'src/entry.js'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
  t.assert.match(r.stderr, /Built src\/entry\.js/)
  t.assert.equal(runNode(t, join(tmp, 'out.js')).trim(), 'hello, world')
}))

test('build rejects an entry that is not an entry point of the artifact', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  t.assert.equal(runCli(['bundle', '--output=app.code.br', 'src/entry.js', 'src/hello.js'], { cwd: tmp }).status, 0)

  const r = runCli(['build', '--output=out.js', 'app.code.br', 'src/nope.js'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
  t.assert.match(r.stderr, /is not an entry point/)
  t.assert.match(r.stderr, /src\/entry\.js/)
}))

test('build requires an artifact and takes at most one entry', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  let r = runCli(['build'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
  t.assert.match(r.stderr, /Nothing to build/)

  r = runCli(['build', 'a.code.br', 'b.js', 'c.js'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
...
test('build supports JSX/TSX entries', withTmp((t, tmp) => {
...
test('build uses esbuild bundler-convention CJS interop, NOT Node\'s (known limitation)', withTmp((t, tmp) => {
  ...
  const r = runCli(['build', '--output=out.mjs', '--format=esm', 'app.code.br'], { cwd: tmp })
  t.assert.equal(r.status, 0, r.stderr)
...
test('build rejects an invalid --format', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
  const r = runCli(['build', '--format=umd', 'stasis.lock.json'], { cwd: tmp })
  t.assert.notEqual(r.status, 0)
...
test('bundleFromLockfile reconstructs a Bundle whose sources match disk', (t) => {
...
test('bundleFromLockfile throws when an attested file is missing on disk', withTmp((t, tmp) => {
  cpSync(fullFixture, tmp, { recursive: true })
...
test('bundleFromLockfile requires a recorded import graph', (t) => {
