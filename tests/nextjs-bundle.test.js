...
test('CLI: stasis bundle --nextjs builds from the fixture with no entry positionals', withTmp(async (t, tmp) => {
  ...
  t.assert.match(r.stderr, /Bundled 16 files in 3 packages/)
...
test('discoverNextEntries: src/ variants, root precedence, and client-entry classification', withTmp(async (t, tmp) => {
  ...
  t.assert.deepEqual(entries, [
    ...
    'src/app/icon2.tsx', // numeric-suffixed metadata image
  ...
  // Client seeds: pages minus api/** and _document (middleware/instrumentation/app never seed).
  t.assert.deepEqual(clientEntries, ['src/pages/_app.tsx', 'src/pages/index.tsx'])
}))
