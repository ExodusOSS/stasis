...
test('run --update --full records a CJS entry and its require()d files idempotently', (t) => {
  const lockPath = join(fixture, 'stasis.lock.json')
  const before = readFileSync(lockPath, 'utf-8')

  const r = run(['run', '--update', '--full', 'src/entry.cjs'], { cwd: fixture })
...
test('run --frozen --full replays a CJS program from the committed lockfile', (t) => {
  const r = run(['run', '--frozen', '--full', 'src/entry.cjs'], { cwd: fixture })
  t.assert.equal(r.status, 0, `stderr: ${r.stderr}`)
  t.assert.equal(r.stdout, 'hello, world\n')
})
...
test('run --bundle=save records commonjs format for CJS files', withTmp((t, tmp) => {
  ...
  const r = run(
    ['run', '--update', '--full', '--bundle=save', `--bundle-file=${bundlePath}`, 'src/entry.cjs'],
    { cwd: fixture }
  )
  t.assert.equal(r.status, 0, `stderr: ${r.stderr}`)
  t.assert.equal(r.stdout, 'hello, world\n')
...
test('run --bundle=load executes a CJS program from a saved bundle', withTmp((t, tmp) => {
  ...
  const load = run(
    ['run', '--frozen', '--full', '--bundle=load', `--bundle-file=${bundlePath}`, 'src/entry.cjs'],
    { cwd: fixture }
  )
