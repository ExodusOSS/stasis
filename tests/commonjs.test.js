...
// spawn() + once('close') yields between tests, so the concurrent stasis runs
...
// Probed once at module load via a top-level-awaited async IIFE (run() is async);
// it settles before any test registers, so tests read the resolved boolean directly.
const RESOLVE_RESOLVE_FROM_BUNDLE = await (async () => {
...
describe('CommonJS handling (spawned, concurrent)', { concurrency: CONCURRENCY }, () => {
  test('run --lock=add records a CJS entry and its require()d files idempotently', withTmp(async (t, tmp) => {
    cpSync(fixture, tmp, { recursive: true })
    const lockPath = join(tmp, 'stasis.lock.json')
    const before = readFileSync(lockPath, 'utf-8')

    const r = await run(['run', '--lock=add', 'src/entry.cjs'], { cwd: tmp })
  ...
  test('run --lock=frozen replays a CJS program from the committed lockfile', withTmp(async (t, tmp) => {
    ...
    cpSync(fixture, tmp, { recursive: true })
    const r = await run(['run', '--lock=frozen', 'src/entry.cjs'], { cwd: tmp })
    t.assert.equal(r.status, 0, `stderr: ${r.stderr}`)
    t.assert.equal(r.stdout, 'hello, world\n')
  }))

  test('run --bundle=add records commonjs format for CJS files', withTmp(async (t, tmp) => {
    cpSync(fixture, tmp, { recursive: true })
  ...
  test('run --bundle=load executes a CJS program from a saved bundle', withTmp(async (t, tmp) => {
    cpSync(fixture, tmp, { recursive: true })
    const bundlePath = join(tmp, 'snapshot.br')
  ...
  test('run --bundle=load executes a CJS program with the entry AND its require()d files missing on disk', withTmp(async (t, tmp) => {
    cpSync(fixture, tmp, { recursive: true })
    const bundlePath = join(tmp, 'snapshot.br')
  ...
  test('run --bundle=load serves an ESM->CJS->CJS node_modules graph with node_modules removed', withTmp(async (t, tmp) => {
  ...
  // load-time fallback.
  test('run --bundle=load resolves a cache-shared sibling require from the bundle', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=add records a sibling importer\'s require() edge (no under-recording)', withTmp(async (t, tmp) => {
    ...
    const bundlePath = join(tmp, 'snapshot.br')
    const save = await run(['run', '--lock=add', '--bundle=add', `--bundle-file=${bundlePath}`, 'index.mjs'], { cwd: tmp })
    t.assert.equal(save.status, 0, `save stderr: ${save.stderr}`)
  ...
  test('run --bundle=load honors package.json main for a cache-shared directory require', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load resolves a cache-shared multi-version bare require to its recorded copy', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load resolves a cache-shared #imports subpath within the importing package', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load resolves a cache-shared workspace #imports subpath from the bundle', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load records and resolves a require.resolve() edge', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load keeps a resolve-only require.resolve() edge whose target is not bundled', withTmp(async (t, tmp) => {
  ...
  test('run --lock=add keys require.resolve() edges under require conditions, not "*"', withTmp(async (t, tmp) => {
    ...
    const save = await run(['run', '--lock=add', 'index.mjs'], { cwd: tmp })
  ...
  test('run --bundle=load gives a bundle-served CJS module a require() with .cache and .extensions (node:59666)', withTmp(async (t, tmp) => {
    ...
    const bundlePath = join(tmp, 'snapshot.br')
    const save = await run(['run', '--lock=add', '--bundle=add', `--bundle-file=${bundlePath}`, 'index.cjs'], { cwd: tmp })
    t.assert.equal(save.status, 0, `save stderr: ${save.stderr}`)
    ...
    const load = await run(['run', '--lock=frozen', '--bundle=load', `--bundle-file=${bundlePath}`, 'index.cjs'], { cwd: tmp })
    t.assert.equal(load.status, 0, `load stderr: ${load.stderr}`)
  ...
  test('run --bundle=load survives import-fresh-style require.cache busting (node:59666)', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load preserves a strict-mode directive behind a leading comment (node:59666)', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load handles a "use strict" directive trailed by a comment without crashing (node:59666)', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load leaves a leading string EXPRESSION intact (node:59666 over-skip)', withTmp(async (t, tmp) => {
  ...
  // expression.
  test('run --bundle=load leaves a multi-line leading string expression intact (node:59666 over-skip)', withTmp(async (t, tmp) => {
  ...
  test('run --bundle=load leaves a leading string continued by `in`/`instanceof` intact (node:59666 over-skip)', withTmp(async (t, tmp) => {
    ...
    const bundlePath = join(tmp, 'snapshot.br')
    const save = await run(['run', '--lock=none', '--bundle=add', `--bundle-file=${bundlePath}`, 'index.cjs'], { cwd: tmp })
    t.assert.equal(save.status, 0, `save stderr: ${save.stderr}`)
    t.assert.equal(save.stdout, 'ok\n')

    rmSync(join(tmp, 'index.cjs'))
    const load = await run(['run', '--lock=none', '--bundle=load', `--bundle-file=${bundlePath}`, 'index.cjs'], { cwd: tmp })
    t.assert.equal(load.status, 0, `load stderr: ${load.stderr}`)
  ...
  test('run --bundle=load keeps strict mode across a multi-line block comment after the directive (node:59666)', withTmp(async (t, tmp) => {
